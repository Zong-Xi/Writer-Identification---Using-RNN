...
# 数据预处理参数设置
path_train = './data/Data/Validation'
path_test = './data/Data/Train'
train_sample = 3000  #训练集中每个同学所有字合并的RHS个数
test_sample = 1000  #测试集中每个同学所有字合并的RHS个数
seq = 100    #单个RHS长度
# 网络参数设置
num_epoch = 7   #训练epoch次数
layer = 1
input_size = 3
hidden_size = 500
batch_size = 200
num_class=107
num_RHS = test_sample
PATH='./model/model_107.pkl'  #保存训练模型位置
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(input_size, hidden_size, layer)  # x_dim,h_dim,layer_num
        self.linear = nn.Linear(hidden_size * seq, num_class)

    def forward(self, x):
        out_gru, h = self.gru(x)
        out_gru = out_gru.view(out_gru.size()[0], -1)
